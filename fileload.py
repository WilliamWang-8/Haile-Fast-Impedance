# does not contain file trim logic for biologic instruments
import pandas as pd
...
def read_chrono(file, source=None, return_tuple=False):
    """
    Read chronopotentiometry data from Gamry .DTA file or Biologic ASCII file
    """
    txt = read_txt(file)
    source = check_source(file, source)

    if source == 'gamry':
            # Gamry .DTA parsing (unchanged)
            cidx = txt.upper().find('\nCURVE\tTABLE') + 1
            if cidx == 0:
                return pd.DataFrame([])
            pretxt = txt[:cidx]
            ...
            usecols = header[1:] if header and header[0]=='' else header
        ...
        # --- read entire file into a DataFrame, using the header row where 'time/s' appears ---
        lines = txt.splitlines()
        header_idx = next((i for i, L in enumerate(lines)
                        if 'time/s' in L.lower()), None)
        ...
        # must have at least time, current, voltage somewhere
        cols = list(data.columns)
        if len(cols) < 3:
            raise ValueError(f"Biologic file did not contain expected columns; found: {cols}")

        # ---- detect by keyword, not position ----
        col_time = next(c for c in cols if 'time' in c.lower())
        col_I    = next(c for c in cols if ('i' in c.lower()) and ('ma' in c.lower()))
        col_E    = next(c for c in cols if 'ewe' in c.lower())
        ...
        # --- 1) calendar strings → elapsed seconds ---
        # if it’s object dtype and contains “/”, treat as full timestamp
        if data['Time'].dtype == object and data['Time'].str.contains('/').any():
            ...
            # fallback to generic parsing if any failed
            if times_dt.isnull().any():
                times_dt = pd.to_datetime(data['Time'], errors='coerce')
            # now overwrite Time with seconds since start
            data['Time'] = (times_dt - times_dt.iloc[0]).dt.total_seconds()
        else:
            # already numeric
            data['Time'] = data['Time'].astype(float)

        # --- 2) convert current from mA → A ---
        data['Im'] = data['Im'].astype(float) * 1e-3

        # --- 3) voltage is already volts, just float it ---
        data['Vf'] = data['Vf'].astype(float)

    # (optional) if you still want an absolute timestamp column:
    # dt0 = get_timestamp(file, source='biologic')
    # data['timestamp'] = dt0 + pd.to_timedelta(data['Time'], unit='s')
